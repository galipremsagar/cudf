...
def query(run_config):
    ...
    # ws_wh: orders shipped from more than one warehouse.
    ...
    # ...and, of those, the ones that also appear in web_returns.
    returned = web_returns.merge(
        ws_wh, left_on="wr_order_number", right_on="ws_order_number"
    )["wr_order_number"]
    ...
    df = df[df["ws_order_number"].isin(ws_wh["ws_order_number"])]
    df = df[df["ws_order_number"].isin(returned)]
