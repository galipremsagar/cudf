...
"""TPC-DS query 8. Store net profit for stores whose ZIP prefix matches selected ZIPs."""
...
def query(run_config):
    ...
    customer_address = pd.read_parquet(
        f"{path}/customer_address{suffix}", columns=["ca_address_sk", "ca_zip"]
    )
    customer_address = customer_address.assign(
        zip5=customer_address["ca_zip"].str.slice(0, 5)
    )

    # The literal ZIP list, deduplicated the way INTERSECT deduplicates.
    listed = customer_address[customer_address["zip5"].isin(_ZIPS)][
        ["zip5"]
    ].drop_duplicates()

    # ZIPs housing more than ten preferred customers.
    customer = pd.read_parquet(
        f"{path}/customer{suffix}",
        columns=["c_current_addr_sk", "c_preferred_cust_flag"],
    )
    preferred = customer[customer["c_preferred_cust_flag"] == "Y"][
        ["c_current_addr_sk"]
    ]
    ...
    selected = selected.assign(zip2=selected["zip5"].str.slice(0, 2))

    # A store is counted once per selected ZIP sharing its two-character prefix.
    store = pd.read_parquet(
        f"{path}/store{suffix}", columns=["s_store_sk", "s_store_name", "s_zip"]
    )
    store = store.assign(zip2=store["s_zip"].str.slice(0, 2))
    store = store.merge(selected[["zip2"]], on="zip2", how="inner")[
    ...
    dates = date_dim[(date_dim["d_qoy"] == 2) & (date_dim["d_year"] == 1998)][
        ["d_date_sk"]
    ]
    ...
    joined = store.merge(
        per_store, left_on="s_store_sk", right_on="ss_store_sk", how="inner"
    )
    grouped = (
        joined.groupby("s_store_name", dropna=False)["ss_net_profit"]
        .sum()
        .reset_index()
    )
    grouped = grouped.sort_values(
        "s_store_name", na_position="first", kind="stable"
    )
